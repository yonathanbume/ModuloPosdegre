...
namespace AKDEMIC.INTRANET.Areas.Dean.Controllers
{
    [Authorize(Roles = ConstantHelpers.ROLES.DEAN)]
    [Area("Dean")]
    [Route("decano/situacion-academica")]
    public class AcademicSituationController : BaseController
    {
        ...
        public AcademicSituationController(
            ...
            IDeanService deanService) : base(termService, userService, dataTablesService) {
        ...
        /// <returns>Vista del historial académico</returns>
        [HttpGet("{id}/historial")]
        public IActionResult AcademicSummary(Guid id) => View(id);

        /// <summary>
        /// Vista donde se muetra el avance curricular del estudiante
        /// </summary>
        /// <param name="id">identificador del estudiante</param>
        ...
        /// </summary>
        /// <param name="cid">Identificador de la facultad</param>
        /// <param name="academicOrder">Orden de mérito</param>
        /// <returns>Listado de estudiantes</returns>
        [HttpGet("get")]
        public async Task<IActionResult> GetStudents(Guid cid, int? academicOrder)
        {
            var userId = _userManager.GetUserId(User);
            var dean = await _deanService.Get(userId);

            var sentParameters = _dataTablesService.GetSentParameters();
            var result = await _studentService.GetStudentDatatableAcademicSituationDean(sentParameters, dean.FacultyId, cid, academicOrder);

            return Ok(result);
        }

        /// <summary>
        /// Genera un reporte del historial académico de un estudiante
        /// </summary>
        /// <param name="id">Identificador del estudiante</param>
