...
namespace AKDEMIC.INTRANET.Areas.Report.Controllers
{
    ...
    [Area("Report")]
    [Route("reporte/asistencia-curso")]
    public class CourseAttendanceStatisticController : BaseController
    {
        private readonly ICourseTermService _courseTermService;

        public CourseAttendanceStatisticController(IDataTablesService dataTablesService, ICourseTermService courseTermService) : base(dataTablesService)
        {
        ...
        public async Task<IActionResult> Get(Guid termId, Guid? careerId, Guid? curriculumId, string search)
        {
            ...
            var result = await _courseTermService.GetCourseAttendanceStatisticsDatatable(sentParameters, termId, careerId, curriculumId, User, search);
            return Ok(result);
        }
        ...
        {
            var result = await _courseTermService.GetCourseAttendanceStatisticsTemplate(termId, careerId, curriculumId, User);
            ...
            foreach (var item in result)
                dt.Rows.Add(item.Code, item.Course, item.Career, item.Mean, item.Median, item.StandardDeviation, item.Percentile25, item.Percentile50, item.Percentile75);

            dt.AcceptChanges();

            string fileName = $"Estadística de asistencia según remuestreo.xlsx";

            using (var wb = new XLWorkbook())
            {
                wb.Worksheets.Add(dt);
                var ws = wb.Worksheet(dt.TableName);
                ws.AddHeaderToWorkSheet("Estadística de asistencia según remuestreo", null);
