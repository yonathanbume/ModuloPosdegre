var reportDetails = function () {
    var datatable = {
        ...
        options: getSimpleDataTableConfiguration({
            url: `/director-carrera/reporte_asistencia/periodo/get`.proto().parseURL(),
            data: function (data) {
                delete data.columns;
    ...
    var detailsDatatable = {
        object: null,
        options: getSimpleDataTableConfiguration({
            url: `/director-carrera/reporte_asistencia/seccion/alumno/get`.proto().parseURL(),
            //url: `/director-carrera/reporte_asistencia/seccion/${sectionId}/alumno/${alumnoId}/get?filter=${$("#assistance-filter").val()}`.proto().parseURL(),
            data: function (data) {
                delete data.columns;
    ...
    var loadTermSelect = function () {
        $("#select2-terms").select2({
            ajax: {
                url: `/director-carrera/reporte_asistencia/periodos/get`.proto().parseURL(),
                delay: 300,
