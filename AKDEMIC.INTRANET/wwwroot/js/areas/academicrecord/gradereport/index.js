var GradeReportManagement = function () {
    ...
    var select = function () {
        $('#careerId').select2({
            ...
            ajax: {
                type: 'GET',
                url: ("/registrosacademicos/informes-de-grados/obtener-carreras").proto().parseURL(),
                delay: 1000,
    ...
    var options = {
        ...
        ajax: {
            type: "GET",
            dataType: "JSON",
            url: `/registrosacademicos/informes-de-grados/obtener-informes-grados`.proto().parseURL(),
        ...
        datatable_init: function () {
            private.objects["tbl-data"].on("click", ".detail",
                function () {
                    ...
                    window.location.href = `/registrosacademicos/informes-de-grados/detalle/${gradereportId}`.proto().parseURL();
            ...
            private.objects["tbl-data"].on("click", ".download",
                function () {
                    ...
                    $btn.addLoader();
                    var gradereportId = $(this).data("id");
                    $.fileDownload(`/registrosacademicos/informes-de-grados/generar-constancia-grado/${gradereportId}`.proto().parseURL())
                        .always(function () {
                            $btn.removeLoader();
                        }).done(function () {
