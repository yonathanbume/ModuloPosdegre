    ...
    var roles = {
        init: function () {
            $.ajax({
                ...
                $.each($("#SelectedRoles2").val().split(","), function (i, e) {
                    $(".select2-roles option[value='" + e + "']").prop("selected", true);
                });
                $(".select2-roles").trigger("change");
            });
        }
    };
    ...
    var datatable = {
        file: {
            ...
            events: {
                ...
                getData: function () {
                    mApp.block("#table_files", {
                        message: "Cargando archivos..."
                    });

                    $.ajax({
                        url: `/admin/eventos/get-archivos?eventId=${EventId}`,
                        type: "get"
                    }).done(function (data) {
                        datatable.file.object.rows().remove().draw();
                        datatable.file.localData = [];
                        ...
                        for (var i = 0; i < data.length; i++) {
                            ...
                            datatable.file.object.row.add([
                                file.name,
                                `<a href="/documentos/${file.url}" target="_blank" class="btn btn-primary btn-sm m-btn m-btn--icon" title="Descargar"><span><i class="la la-download"></i><span>Descargar</span></span></a> ` +
                                `<button type="button" data-id="${file.id}" class="btn btn-danger btn-sm m-btn m-btn--icon btn-delete" title="Eliminar"><span><i class="la la-trash"></i><span>Eliminar</span></span></button>`
                            ]).draw(false);
                        }

                        mApp.unblock("#table_files");
                    });
                },
                init: function () {
                    this.onDelete();
        ...
        formCreate = $("#create-form").validate({
            ...
            submitHandler: function (formElements, e) {
                ...
                for (var i = 0; i < datatable.file.localData.length; i++) {
                    ...
                    formData.append(`EventFiles[${i}].ToDo`, datatable.file.localData[i].toDo);
                }
