var CourseTable = function () {
    ...
    var options = {//getSimpleDataTableConfiguration({
        ajax: {
            type: "GET",
            dataType: "JSON",
            url: "/admin/examen-subsanacion/get-datatable".proto().parseURL(),
            data: function (data) {
                delete data.columns;
        ...
        columns: [
            ...
            },
            {
                ...
                render: function (row) {
                    var tmp = "";
                    tmp += `<a href="/admin/examen-subsanacion/${row.id}/gestionar" class="btn btn-primary btn-sm m-btn  m-btn m-btn--icon"><span><i class="la la-cog"></i><span>Gestionar</span></span></a>`;
    ...
    var selectsModal = {
        ...
        teacher: {
            init: function () {
                $("#create_modal").find("[name='TeacherId']").select2({
                    ajax: {
                        delay: 1000,
