var MedicalCare = function () {
    ...
    var defaultSettings = {
        ...
        eventRender: function (event, element) {
                ...
                swal({
                    ...
                    preConfirm: () => {
                        return new Promise((resolve) => {
                            $.ajax({
                                url: `/topico/horario-citas/eliminar/${id}`.proto().parseURL(),
                                type: "POST",
            ...
            element.find(".edit").click(function (e) {
                ...
                $.ajax({
                    type: 'GET',
                    url: `/topico/horario-citas/obtener/${id}`.proto().parseURL(),
                    success: function (data) {
                        if (data.studentid !== null) {
                            $("#swichPersonalized2").trigger('click');
                        }
                        $("#modal-edit-topic select[id='StudentId2']").val(data.studentid).trigger('change');
                        $("#modal-edit-topic input[id='Id']").val(data.id);
                        $("#modal-edit-topic input[id='DateMedicalCare2']").val(data.datemedicalcare);
                        $("#modal-edit-topic input[id='TimeMedicalCare2']").val(data.timemedicalcare);
                    }
        ...
        events: {
            ...
            url: ("/topico/horario-citas/get").proto().parseURL()
        },
        ...
        eventClick: function (calEvent, jsEvent, view) {
            ...
            if (calEvent.studentId == _app.constants.guid || calEvent.studentId == null)
                ...
                location.href = `/topico/horario-citas/detalle-historico/${calEvent.id}/${calEvent.studentId}/${calEvent.docId}`.proto().parseURL();
            }
    ...
    var modal = function () {
        ...
        $("#edit-topic-modal").on("hidden.bs.modal", function () {
            $("#swichPersonalized2").trigger('click');
    ...
    var select2Students = function () {
        ...
        $("#modal-create-medicalappointment").validate({
            submitHandler: function (form, event) {
                ...
                $.ajax({
                    type: 'POST',
                    url: ("/topico/horario-citas/agregar-cita/post").proto().parseURL(),
        ...
        $("#modal-edit-topic").validate({
            submitHandler: function (form, event) {
                $.ajax({
                    type: 'POST',
                    url: ("/topico/horario-citas/editar-cita/post").proto().parseURL(),
                    data: $("#modal-edit-topic").serialize(),
                    success: function (data) {
