var NutritionalCare = function () {
    ...
    var defaultSettings = {
        ...
        eventRender: function (event, element) {
                ...
                swal({
                    ...
                    preConfirm: () => {
                        return new Promise((resolve) => {
                            $.ajax({
                                url: `/nutricion/horario-citas/eliminar/${id}`.proto().parseURL(),
                                type: "POST",
            ...
            element.find(".edit").click(function (e) {
                e.stopPropagation();
                var id = $(this).data("id");
                $("#edit-nutritionalappointment-modal").modal('show');

                $.ajax({
                    type: 'GET',
                    url: `/nutricion/horario-citas/obtener/${id}`.proto().parseURL(),
                    success: function (data) {
                        if (data.studentid !== null) {
                            $("#swichPersonalized2").trigger('click');
                        }
                        $("#modal-edit-nutritionalappointment select[id='StudentId2']").val(data.studentid).trigger('change');
                        $("#modal-edit-nutritionalappointment input[id='Id']").val(data.id);
                        $("#modal-edit-nutritionalappointment input[id='DateMedicalCare2']").val(data.datemedicalcare);
                        $("#modal-edit-nutritionalappointment input[id='TimeMedicalCare2']").val(data.timemedicalcare);
                    }
        ...
        events: {
            ...
            url: ("/nutricion/horario-citas/get").proto().parseURL()
        },
        ...
        eventClick: function (calEvent, jsEvent, view) {
            ...
            if (calEvent.studentId === _app.constants.guid || calEvent.studentId === null)
                ...
                location.href = `/nutricion/horario-citas/detalle-historico/${calEvent.id}`.proto().parseURL();
            }
        },
        firstDay: 1,
        header: {
    ...
    var modal = function () {
        $("#btnAdd").on('click', function () {
            $("#add-nutritionalappointment-modal").modal('show');
        });
        $("#edit-nutritionalappointment-modal").on("hidden.bs.modal", function () {
            $("#swichPersonalized2").trigger('click');
            $("#edit-nutritionalappointment-modal").modal('hide');
        });
    };
    var select2Students = function () {
        ...
        $("#modal-create-nutritionalappointment").validate({
            submitHandler: function (form, event) {
                //$(":submit").addLoader();
                $.ajax({
                    type: 'POST',
                    url: ("/nutricion/horario-citas/agregar-cita/post").proto().parseURL(),
                    data: $("#modal-create-nutritionalappointment").serialize(),
                    success: function (data) {
                        toastr.success(_app.constants.toastr.message.success.create, _app.constants.toastr.title.update);
                        $("#add-nutritionalappointment-modal").modal("hide");
                        if (fullCalendar !== null) {
                            $(fullCalendar).fullCalendar("destroy");
                            fullCalendar = null;
                        }
        ...
        $("#modal-edit-nutritionalappointment").validate({
            submitHandler: function (form, event) {
                $.ajax({
                    type: 'POST',
                    url: ("/nutricion/horario-citas/editar-cita/post").proto().parseURL(),
                    data: $("#modal-edit-nutritionalappointment").serialize(),
                    success: function (data) {
