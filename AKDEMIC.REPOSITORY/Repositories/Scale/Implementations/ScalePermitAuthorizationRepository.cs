...
{
    public class ScalePermitAuthorizationRepository : Repository<ScalePermitAuthorization>, IScalePermitAuthorizationRepository
    {
        public ScalePermitAuthorizationRepository(AkdemicContext context) : base(context) { }

        public async Task<int> GetPermitAuthorizationsQuantity(string userId)
        {
            var records = await _context.ScalePermitAuthorizations
        ...
        {
            var query = _context.ScalePermitAuthorizations
                .Where(x => x.UserId == userId)
                .AsQueryable();
            ...
            var pagedList = await query.Skip(paginationParameter.CurrentNumber).Take(paginationParameter.RecordsPerPage)
                .Select(x => new ScalePermitAuthorization
                {
                    Id = x.Id,
        ...
        public async Task<Tuple<int, List<ScalePermitAuthorization>>> GetPermitAuthorizationsReportByPaginationParameters(PaginationParameter paginationParameter, ClaimsPrincipal user)
        {
            var query = _context.ScalePermitAuthorizations
            ...
            return new Tuple<int, List<ScalePermitAuthorization>>(records, pagedList);
        }
