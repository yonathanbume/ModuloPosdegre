...
{
    public class WorkerTechnicalStudyRepository : Repository<WorkerTechnicalStudy>, IWorkerTechnicalStudyRepository
    {
        public WorkerTechnicalStudyRepository(AkdemicContext context) : base(context) { }

        public async Task<List<WorkerTechnicalStudy>> GetAllByUserId(string userId)
        {
            return await _context.WorkerTechnicalStudies.Include(x => x.Institution).Where(x => x.UserId == userId).ToListAsync();
        }

        public async Task<int> GetWorkerTechnicalStudiesQuantity(string userId)
        {
            var records = await _context.WorkerTechnicalStudies
        ...
        public async Task<List<TechnicalStudiesTemplate>> GetWorkerTechnicalStudiesByPaginationParameters(string userId, PaginationParameter paginationParameter)
        {
            ...
            var pagedList = await query.Skip(paginationParameter.CurrentNumber).Take(paginationParameter.RecordsPerPage)
                .Select(x => new TechnicalStudiesTemplate
                {
                    Id = x.Id,
        ...
        {
            var query = await _context.WorkerTechnicalStudies.Include(x => x.Institution).Where(x => x.Id == id).FirstOrDefaultAsync();
