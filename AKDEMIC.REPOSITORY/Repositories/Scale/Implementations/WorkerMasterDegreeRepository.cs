...
{
    public class WorkerMasterDegreeRepository : Repository<WorkerMasterDegree>, IWorkerMasterDegreeRepository
    {
        public WorkerMasterDegreeRepository(AkdemicContext context) : base(context) { }

        public async Task<List<WorkerMasterDegree>> GetAllByUserId(string userId)
        {
            return await _context.WorkerMasterDegrees.Include(x => x.Institution).Where(x => x.UserId == userId).AsQueryable().ToListAsync();
        }

        public async Task<int> GetWorkerMasterDegreesQuantity(string userId)
        {
            var records = await _context.WorkerMasterDegrees
        ...
        public async Task<List<MasterDegreesTemplate>> GetWorkerMasterDegreesByPaginationParameters(string userId, PaginationParameter paginationParameter)
        {
            ...
            var pagedList = await query.Skip(paginationParameter.CurrentNumber).Take(paginationParameter.RecordsPerPage)
                .Select(x => new MasterDegreesTemplate
                {
                    Id = x.Id,
                    Name = x.Name,
        ...
        {
            var query = await _context.WorkerMasterDegrees.Include(x => x.Institution).Where(x => x.Id == id).FirstOrDefaultAsync();
