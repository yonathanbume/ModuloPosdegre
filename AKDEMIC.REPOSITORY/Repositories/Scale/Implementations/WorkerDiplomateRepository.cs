...
{
    public class WorkerDiplomateRepository : Repository<WorkerDiplomate>, IWorkerDiplomateRepository
    {
        public WorkerDiplomateRepository(AkdemicContext context) : base(context) { }

        public async Task<List<WorkerDiplomate>> GetAllByUserId(string userId)
        {
            return await _context.WorkerDiplomates.Where(x => x.UserId == userId).AsQueryable().ToListAsync();
        }

        public async Task<int> GetWorkerDiplomatesQuantity(string userId)
        {
            var records = await _context.WorkerDiplomates
        ...
        {
            var query = _context.WorkerDiplomates
                .Where(x => x.UserId == userId)
                .AsQueryable();
            ...
            var pagedList = await query.Skip(paginationParameter.CurrentNumber).Take(paginationParameter.RecordsPerPage)
                .Select(x => new DiplomatesTemplate
                {
                    Id = x.Id,
        ...
        {
            var query = await _context.WorkerDiplomates.Include(x => x.Institution).Where(x => x.Id == id).FirstOrDefaultAsync();
