...
namespace AKDEMIC.REPOSITORY.Repositories.Scale.Implementations
{
    ...
    {
        public BenefitTypeRepository(AkdemicContext context) : base(context) { }

        public async Task<bool> AnyByName(string name, Guid? id = null)
        {
            return await _context.BenefitTypes.AnyAsync(x => x.Name.ToUpper() == name.ToUpper() && x.Id != id);
        }
        ...
        {
            Expression<Func<BenefitType, dynamic>> orderByPredicate = null;
            ...
            var query = _context.BenefitTypes.AsNoTracking();
