...
{
    public class WorkerSecondSpecialtyRepository : Repository<WorkerSecondSpecialty>, IWorkerSecondSpecialtyRepository
    {
        public WorkerSecondSpecialtyRepository(AkdemicContext context) : base(context) { }

        public async Task<List<WorkerSecondSpecialty>> GetAllByUserId(string userId)
        {
            return await _context.WorkerSecondSpecialties.Include(x => x.Institution).Where(x => x.UserId == userId).AsQueryable().ToListAsync();
        }

        public async Task<int> GetWorkerSecondSpecialtiesQuantity(string userId)
        {
            var records = await _context.WorkerSecondSpecialties
        ...
        {
            var query = _context.WorkerSecondSpecialties
                .Where(x => x.UserId == userId)
                .AsQueryable();
            ...
            var pagedList = await query.Skip(paginationParameter.CurrentNumber).Take(paginationParameter.RecordsPerPage)
                .Select(x => new SecondSpecialtiesTemplate
                {
                    Id = x.Id,
        ...
        {
            var query = await _context.WorkerSecondSpecialties.Include(x => x.Institution).Where(x => x.Id == id).FirstOrDefaultAsync();
            return query;
        }
    }
}
