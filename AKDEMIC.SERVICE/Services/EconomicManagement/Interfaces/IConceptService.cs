...
namespace AKDEMIC.SERVICE.Services.EconomicManagement.Interfaces
{
    ...
    {
        Task<Concept> GetConcept(Guid id);
        Task<Concept> Add(Concept concept);
        Task Update();
        Task InsertRange(List<Concept> concepts);
        Task AddRange(List<Concept> concepts);

        Task InsertConcept(Concept concept);

        Task UpdateConcept(Concept concept);

        Task DeleteConcept(Concept concept);

        Task<IEnumerable<Concept>> GetAllWithIncludes();
        Task<IEnumerable<Concept>> GetConcepts();
        ...
        Task<DataTablesStructs.ReturnedData<object>> GetConceptsDatatable(DataTablesStructs.SentParameters sentParameters, ClaimsPrincipal user = null, string search = null, string code = null, byte? status = null, Guid? classifierId = null, Guid? dependencyId = null, bool? showTaxedOnly = null);
