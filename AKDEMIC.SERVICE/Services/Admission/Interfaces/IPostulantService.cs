...
namespace AKDEMIC.SERVICE.Services.Admission.Interfaces
{
    ...
    {
        Task<Postulant> Get(Guid id);
        Task AddAsync(Postulant postulant);
        Task AddRangeAsync(List<Postulant> postulants);
        Task InsertPostulant(Postulant postulant);
        Task InsertPostulant(Postulant postulant, bool generateCode = false);
        Task UpdateRange(IEnumerable<Postulant> postulants);
        Task UpdatePostulant(Postulant postulant);
        Task DeletePostulant(Postulant postulant);
        Task<Postulant> GetPostulantById(Guid id);
        Task<IEnumerable<Postulant>> GetAllPostulants();
        Task<List<Postulant>> GetAllPostulantsWithFolders();
        Task<IEnumerable<Postulant>> GetAllByFacultyAndCareerAndTerm(Guid? facultyId = null, Guid? careerId = null,
            Guid? termId = null, byte? admissionState = null);
